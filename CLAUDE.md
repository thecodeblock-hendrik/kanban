...
### Backend: single-file FastAPI app (`backend/app/main.py`)
Everything — DB schema/init, board CRUD, AI integration, static serving — lives in this one module; there is no separate models/routes/services split yet. Key pieces:
- **DB**: raw `sqlite3` (no ORM). Tables: `users`, `boards`, `board_columns`, `board_cards`. `init_db()` creates tables and seeds the hardcoded `user`/`password` row on startup. `get_or_create_user_board()` lazily creates a user's board (and default 5-column/8-card layout from `DEFAULT_COLUMNS`/`DEFAULT_CARDS`) on first access.
...
- **Persistence**: `replace_board_state()` fully validates the incoming board (`validate_board_state()` — checks for duplicate/orphaned/missing card ids, non-empty titles, etc.) before deleting and rewriting `board_columns`/`board_cards` for that board in one connection/transaction. Never persist an unvalidated payload.
- **AI integration**: `call_openrouter()` hits OpenRouter (`openai/gpt-oss-120b`) with a plain `urllib` POST (no SDK). `OPENROUTER_API_KEY` is read from the root `.env` via a minimal hand-rolled `load_env()` (not python-dotenv). `POST /api/ai/board` builds a prompt containing the full current board JSON + chat history (`build_board_prompt()`), requires the model to return structured JSON (`{"response": ..., "board_update": {...}|omitted}`), and `parse_ai_board_response()`/`validate_ai_board_update()` strictly validate that shape before any board mutation is applied via `replace_board_state()`. Treat model output as untrusted input.
- Routes: `GET /api/health`, `GET /api/ai/test` (quick OpenRouter smoke test), `POST /api/ai/board`, `GET/PUT /api/board?user=...` (user passed as a query param, not real auth), and the `/` static mount.
...
- `components/KanbanBoard.tsx` — the stateful orchestrator: loads the board from `GET /api/board` on mount, wires `@dnd-kit` drag events through `moveCard()`, and persists every board change via `PUT /api/board`. Persistence uses a `saveSequenceRef` counter to drop stale/out-of-order save responses (a superseded save's response is ignored) rather than a debounce — be aware of this pattern before changing the save effect. Also owns the AI chat sidebar state (`chatMessages`, calls `POST /api/ai/board`) and applies any returned `board` directly to state.
